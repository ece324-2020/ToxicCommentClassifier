...
# basically the same as multi_loose, but generates 1000 per category
...
sample_good = good.sample(n = 1000, random_state = seed, replace = False) #choose 478 because the maximum for threat is 478, want to balance every class
...
# print(severe_toxic["severe_toxic"].value_counts()) #0 - all 1000 need to come from multi labelled
# print(obscene["obscene"].value_counts()) #317 - 683 from multi labelled
# print(threat["threat"].value_counts()) #22 - all 1000 need to come from multi labelled (with repeats)
# print(insult["insult"].value_counts()) #301 - 699 from multi labelled
# print(identity_hate["identity_hate"].value_counts()) #54 - 946 from multi labelled

#custom selections based on results above
toxic           = toxic.sample(n = 1000, random_state = seed, replace = False)
# print(toxic.shape) #make sure all classes are 1000

severe_toxic    = train[(train["severe_toxic"]==1)].sample(n = 1000, random_state = seed, replace = False)
severe_toxic    = severe_toxic.assign(toxic=0, obscene=0, threat=0, insult=0, identity_hate=0)
# print(severe_toxic.shape)

obscene2        = train[(train["obscene"]==1)].sample(n = 683, random_state = seed, replace = False)
obscene2        = obscene2.assign(toxic=0, severe_toxic=0, threat=0, insult=0, identity_hate=0)
obscene         = pd.concat([obscene, obscene2])
# print(obscene.shape)

threat          = train[(train["threat"]==1)].sample(n = 1000, random_state = seed, replace = True)
threat          = threat.assign(toxic=0, severe_toxic=0, obscene=0, insult=0, identity_hate=0)
# print(threat.shape)

insult2         = train[(train["insult"]==1)].sample(n = 699, random_state = seed, replace = False)
insult2         = insult2.assign(toxic=0, severe_toxic=0, obscene=0, threat=0, identity_hate=0)
insult          = pd.concat([insult, insult2])
# print(insult.shape)

identity_hate2  = train[(train["identity_hate"]==1)].sample(n = 946, random_state = seed, replace = False)
...
# print(bad.shape)
print(bad["severe_toxic"].value_counts())
...
train.sample(frac=1).to_csv("./multi_loose_large_data/train.csv", index=False)
valid.sample(frac=1).to_csv("./multi_loose_large_data/valid.csv", index=False)
test.sample(frac=1).to_csv("./multi_loose_large_data/test.csv", index=False)
overfit.sample(frac=1).to_csv("./multi_loose_large_data/overfit.csv", index=False)